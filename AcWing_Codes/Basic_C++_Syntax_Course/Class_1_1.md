...
* i++与++i的详细区别：
  ...
  > 具体实现：
    > i++：
    ![i++的具体实现](images/2023-02-26-10-43-56.png)
    > ++i:
    ![++i的具体实现](images/2023-02-26-10-45-19.png)
  ...
  > cout的执行顺序：
    ...
    > ![示例代码](images/2023-02-26-10-53-12.png)
        > 初始值i = 0；
        > `cout << "i++:" << i++ << "i++" << i++ << endl`
            > 先扫描到第二个i++，i变为1，此时i++的返回值为0，则0进入缓冲区：|0|.
            > 然后扫描到第二个i++，i变为2，此时i++的返回值为1，则1进入缓冲区：|0||1|.
            > 输出|1| |0|.此时i = 2.
        > `cout << "++i:" << ++i << ",++i ;" << ++i << endl`
            > 先扫描到第二个++i，i变为3，此时返回i的引用，缓冲区：|&i|.
            > 然后扫描到第一个++i，i变为4，此时返回i的引用，缓冲区：|&i||&i|.
            > 输出时，i此时的值为4，因此输出的时候从i对应的地址中读取数据，则输出：4 4.此时i = 4；
            > 
* 代码示例：
